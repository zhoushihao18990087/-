...
import com.example.group.util.Utils1;
...
public class shoppingcart extends AppCompatActivity implements View.OnClickListener {
    ...
    // 声明一个根据商品编号查找商品信息的映射
    private HashMap<Long, GoodsInfo1> mGoodsMap = new HashMap<Long, GoodsInfo1>();
    ...
    private void showCart() {
        ...
        for (int i = 0; i < mCartArray.size(); i++) {
            ...
            // 根据商品编号查询商品数据库中的商品记录
            GoodsInfo1 goods = mGoodsHelper.queryById(info.goods_id);
          //  Log.d(TAG, "name=" + goods.name + ",price=" + goods.price + ",desc=" + goods.desc);
            ...
            LinearLayout.LayoutParams iv_params = new LinearLayout.LayoutParams(
                    0, Utils1.dip2px(this, 85), 2);
            iv_thumb.setLayoutParams(iv_params);
            iv_thumb.setScaleType(ImageView.ScaleType.FIT_CENTER);
            iv_thumb.setImageBitmap(MyApplication.getInstance().mIconMap.get(info.goods_id));
    ...
    private void refreshTotalPrice() {
        ...
        for (CartInfo info : mCartArray) {
            GoodsInfo1 goods = mGoodsMap.get(info.goods_id);
